...
// TODO control the position of all objects
...
function createScene(canvas: HTMLCanvasElement) {
  ...
  const map = new THREE.TextureLoader().load(
    `${IMAGE_BASEURL}/ash_uvgrid01.jpg`,
  );
  ...
  sphereGroup.position.set(10, 30, -40);

  // 4. Create sphere
  sphere = new THREE.Mesh(new THREE.SphereGeometry(5, 20, 20), material);
  sphereGroup.add(sphere);

  // 5. Create Cone
  cone = new THREE.Mesh(
    new THREE.CylinderGeometry(0, 0.333, 0.444, 20, 5),
    material,
  );
  // 将圆锥放到球体的上方
